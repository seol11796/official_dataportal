...
router.post('/', util.isLoggedin, upload.single('attachment'), async function(req, res){
  ...
  Usage.create(req.body, function(err, usage){
    ...
    res.redirect('/usage'+res.locals.getPostQueryString(false, { page:1, searchText:'' }));
...
router.put('/:id', util.isLoggedin, checkPermission, upload.single('newAttachment'), async function(req, res){
  var usage = await Usage.findOne({_id:req.params.id}).populate({path:'attachment',match:{isDeleted:false}});
  if(usage.attachment && (req.file || !req.body.attachment)){
    usage.attachment.processDelete();
  }
  req.body.attachment = req.file?await File.createNewInstance(req.file, req.user._id, req.params.id):usage.attachment;
  req.body.updatedAt = Date.now();
  Usage.findOneAndUpdate({_id:req.params.id}, req.body, {runValidators:true}, function(err, usage){
    if(err){
      req.flash('usage', req.body);
      req.flash('errors', util.parseError(err));
      return res.redirect('/usage/'+req.params.id+'/edit'+res.locals.getPostQueryString());
    }
    res.redirect('/usage/'+req.params.id+res.locals.getPostQueryString());
  });
});

// destroy
router.delete('/:id', util.isLoggedin, checkPermission, function(req, res){
  Usage.deleteOne({_id:req.params.id}, function(err){
    if(err) return res.json(err);
...
function checkPermission(req, res, next){
  Usage.findOne({_id:req.params.id}, function(err, usage){
    if(err) return res.json(err);
