...
router.post("/", util.isLoggedin, function (req, res) {
  ...
  Application.create(req.body, function (err, post) {
    if (err) {
      ...
      return res.redirect("/application/new" + res.locals.getPostQueryString());
    }
    res.redirect(
      "/application" +
...
router.get("/:id", function (req, res) {
  Promise.all([
    Application.findOne({ numId: req.params.id }).populate({
      path: "author",
      select: "username",
    }),
  ])
...
router.get("/:id/edit", util.isLoggedin, checkPermission, function (req, res) {
  ...
  var errors = req.flash("errors")[0] || {};
  if (!post) {
    Application.findOne({ numId: req.params.id }, function (err, post) {
...
router.put("/:id", util.isLoggedin, checkPermission, function (req, res) {
  ...
  Application.findOneAndUpdate(
    ...
    function (err, post) {
      ...
      res.redirect(
        "/application/" + req.params.id + res.locals.getPostQueryString()
      );
    }
  );
});
...
router.delete("/:id", util.isLoggedin, checkPermission, function (req, res) {
  Application.deleteOne({ numId: req.params.id }, function (err) {
    if (err) return res.json(err);
